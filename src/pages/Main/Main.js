    ...
    changeView = (viewName) => {
        ...
        if(viewName === 'Login'){
            Alert.alert('Tem certeza que deseja sair?', 'Você será deslogado e enviado para a tela de login', [
                {text: 'Voltar'},
                {text: 'Sair', onPress: () => {
                    setUserToLoggedOut().then(() => this.props.navigation.navigate(viewName, this.props.navigation.navigate))
                }}
            ])
            ...
            Alert.alert('Histórico de respostas indisponível', 'Você entrou sem fazer login. Para visualizar seu histórico salvo, saia e faça login.', [
                {text: 'Fazer login', onPress: () => {
                    setUserToLoggedOut().then(() => this.props.navigation.navigate('Login', this.props.navigation.navigate))
                    }},
                {text: 'Ok'}
            ])
        }else{
